...
import { 
  ...
  FaMapMarkerAlt, FaStar, FaRegStar, FaRegHeart, FaRegBell, FaGlobe,
  FaMoon, FaSun, FaShoppingCart, FaBookmark, FaChevronDown
} from 'react-icons/fa';
...
function Navbar() {
  ...
  return (
    <>
      <motion.nav
        className={`bg-gradient-to-r from-blue-100 to-blue-500 text-blue-900 py-1 px-4 shadow-lg sticky top-0 z-40 transition-all duration-300 ${darkMode ? 'dark' : ''}`}
        initial={{ y: -100 }}
        animate={{ y: 0 }}
      ...
      >
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center">
            ...
            >
              <FaHotel className="mr-2 text-blue-900" size={20} />
              <Link
                to="/"
                className="text-xl font-extrabold tracking-wide text-blue-900 drop-shadow-lg"
              >
                OUR Hotels
              </Link>
            </motion.div>
          ...
          >
            <motion.div variants={linkVariants} whileHover="hover" whileTap="tap">
              <Link
                to="/hotels"
                className="text-sm font-medium px-3 py-1 rounded-lg transition-all duration-300 flex items-center text-blue-900 hover:bg-blue-200"
                aria-label="Hotels"
              >
                <FaHotel className="mr-2 text-blue-900" />
                Hotels
              </Link>
            </motion.div>           
            ...
            {user && (
              ...
              >
                <motion.div
                  ...
                  className="p-1 rounded-full hover:bg-blue-200 transition-colors duration-300"
                >
                  <Badge badgeContent={unreadNotifications} color="error">
                    <FaBell size={16} className="text-blue-900" />
                  </Badge>
                </motion.div>
                      ...
                      <React.Fragment key={notification.id}>
                        <ListItem button>
                          <ListItemIcon>
                            ...
                            {notification.title === 'Booking Confirmed' && <FaHotel className="text-blue-700" />}
                          </ListItemIcon>
                          <ListItemText
            ...
            {user && (
              <motion.div variants={linkVariants} whileHover="hover" whileTap="tap">
                <Link
                  ...
                  aria-label="Wishlist"
                >
                  <FaHeart size={16} className="text-blue-900" />
                </Link>
              </motion.div>
            )}
              ...
              <motion.div variants={linkVariants} whileHover="hover" whileTap="tap">
                <div className="flex items-center space-x-2">
                  <Button
                    ...
                    startIcon={<Avatar sx={{ width: 28, height: 28, bgcolor: 'blue.800' }}>
                      {getUserInitials()}
                    </Avatar>}
                    endIcon={<FaChevronDown size={12} className="text-blue-900" />}
                    sx={{
                      color: 'blue.900',
                  ...
                  >
                    <span className="hidden md:inline text-blue-900">{getUserDisplayName()}</span>
                  </Button>
          ...
          <div className="md:hidden flex items-center">
            ...
            >
              {mobileMenuOpen ? <FaTimes size={20} className="text-blue-900" /> : <FaBars size={20} className="text-blue-900" />}
            </motion.button>
          </div>
        </div>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              className="md:hidden bg-blue-50 text-blue-900 mt-2 rounded-lg shadow-lg"
            ...
            >
              <div className="container mx-auto px-4 py-4 flex flex-col space-y-4">
                <Link
                  to="/hotels"
                  className="flex items-center text-lg font-medium py-2 px-3 rounded-lg hover:bg-blue-200"
                ...
                {user && (
                  <>
                    <Link
                      ...
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      <FaHeart className="mr-2 text-blue-700" />
                      Wishlist
                    </Link>
                    <Link
                      to="/order-history"
                      ...
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      <FaBookmark className="mr-2 text-blue-800" />
                      My Bookings
                    </Link>
                    <Link
                      to="/profile"
                      ...
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      <FaUserCircle className="mr-2 text-blue-800" />
                      Profile
                    </Link>
                  </>
                )}
                
                <div className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-blue-200">
                  <div className="flex items-center">
                    <FaMoon className="mr-2 text-blue-800" />
                    <span>Dark Mode</span>
                  </div>
                  <div className="relative inline-block w-10 h-6">
                    ...
                    />
                    <span className={`absolute top-0 left-0 right-0 bottom-0 bg-gray-300 rounded-full transition ${darkMode ? 'bg-blue-700' : ''}`}></span>
